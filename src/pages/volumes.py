...
with st.expander("ℹ️ About Docker Volumes"):
    ...
    **Volume States:**
    - 🟢 **In Use**: Volume is currently mounted to one or more containers
    - ⚪ **Unused**: Volume exists but is not mounted to any container
    ...
    with st.form("create_volume_form"):
        ...
        if submitted:
            try:
                ...
                st.success(f"✅ Volume '{new_volume.name}' created successfully!")
                st.rerun()
            except Exception as e:
                st.error(f"Failed to create volume: {e}")

# List all volumes
st.header(f"📋 All Volumes ({len(volumes)})")
...
if volumes:
    ...
    for volume in sorted_volumes:
        ...
        with st.expander(f"{status_icon} [{status_text}] **{volume.name}**", expanded=is_confirming):
            ...
            if is_used:
                ...
                for container_info in volume_usage[volume.name]:
                    ...
                    with cont_col2:
                        ...
                        if st.button("🔌", key=detach_key, help="Detach volume from this container", use_container_width=True):
                            ...
                            if not st.session_state[confirm_detach_key]:
                                ...
                                try:
                                    ...
                                    st.success(f"✅ Volume '{volume.name}' detached from container '{original_name}'")
                                    st.session_state[confirm_detach_key] = False
                                    st.rerun()
                        ...
                        if st.session_state.get(f"confirm_detach_{volume.name}_{container_info['id']}", False):
                            st.warning("⚠️ Click again to confirm detach")

            
            # Only allow removing if not in use
            if not is_used:
                if st.button("🗑️ Remove", key=f"remove_{volume.name}", type="secondary"):
                    # Confirmation dialog
                    confirm_key = f"confirm_remove_{volume.name}"
            ...
            button_text = "📋 Hide Details" if st.session_state[details_key] else "📋 Show Full Details"
            ...
            st.markdown("---")
            st.markdown("**🔗 Attach to Container**")
            ...
            if st.button("➕ Attach Volume to Container", key=f"show_mount_{volume.name}", use_container_width=True):
                st.session_state[mount_form_key] = not st.session_state[mount_form_key]
                st.rerun()
            
            if st.session_state[mount_form_key]:
                st.info("⚠️ Note: Docker doesn't support attaching volumes to running containers. This will recreate the container with the volume mounted.")
                ...
                with st.form(key=f"mount_form_{volume.name}"):
                    ...
                    if not available_containers:
                        ...
                        with col_submit:
                            submitted = st.form_submit_button("🔄 Recreate Container with Volume", type="primary", use_container_width=True)
                        with col_cancel:
                        ...
                        if submitted and mount_path:
                            ...
                            try:
                                ...
                                st.success(f"✅ Container '{original_name}' recreated with volume '{volume.name}' mounted at '{mount_path}'")
                                st.session_state[mount_form_key] = False
                                st.rerun()
